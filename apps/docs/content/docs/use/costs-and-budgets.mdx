...
<Callout type="info" title="Where to verify">
Open **Agents > [agent]** and scroll to the **Inference Costs** section. Budget limits are displayed alongside current spend for the active period. You can add, edit, or remove limits directly from this panel.
</Callout>
...
The model catalog in **Settings > Gateway** shows per-token pricing for every model available to your instance. Check it before switching an agent to a paid model.
...
<Callout type="info" title="Where to verify">
Open **Activity > [work item]** to see the timeline of inference calls for a specific run. Each call shows token counts, cost, latency, and the model used. The same data rolls up into **Costs** at the aggregate level.
</Callout>
...
No separate billing system is needed. All cost data lives in the app, computed from the receipts stored alongside every run.
