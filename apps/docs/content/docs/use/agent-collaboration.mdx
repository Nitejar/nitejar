...
<Callout type="info" title="Where to verify">
  Open **Activity** to see per-message triage decisions, exclusive claims, and silent passes. Each
  work item shows which agent responded and which agents evaluated but passed. The activity timeline
  is the source of truth for collaboration behavior.
</Callout>
