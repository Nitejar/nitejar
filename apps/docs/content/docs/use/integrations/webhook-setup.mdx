...
<Callout type="info" title="Where to verify">
  Open **Activity** to see the work item and the agent's response. Click into the work item for the
  full receipt trail: webhook received, run dispatched, inference calls, tools executed, response
  delivered. Each step has timestamps and cost.
</Callout>
