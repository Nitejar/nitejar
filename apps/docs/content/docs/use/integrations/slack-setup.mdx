...
5. Optionally choose your **Listen for** mode and whether to enable **agent mention handoffs** (both can be changed later).
...
### Agent mention handoffs

When **Enable agent mention handoffs** is on, an agent's final response can trigger another agent by using explicit transfer language like "handoff to @agent" or "@agent can you take this?"

Nitejar detects the transfer intent and creates a follow-up work item for the mentioned agent:

- The original message starts at chain depth `0`.
- Each handoff increments the depth by `1`.
- Maximum chain depth is **3** to prevent runaway loops.

Only explicit transfer phrasing triggers a handoff. Casual or referential mentions (like "as @agent mentioned earlier") do not.
