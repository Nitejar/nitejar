...
**Production:** Set your public URL in **Settings > Runtime > Public app URL**, or set `APP_BASE_URL` in your environment. Nitejar auto-registers the webhook with Telegram when you save the plugin instance. The runtime setting takes priority over environment variables. See the [configuration docs](/start-here/configuration#public-app-url) for the full resolution order.
...
Or set it in **Settings > Runtime > Public app URL**.
...
4. Open **Activity** to see the work item. Click it to inspect the full run: webhook payload, inference calls, tool executions, token counts, and the delivered response.

If the bot does not respond, see [Troubleshooting](#7-troubleshooting) below.

<Callout type="info" title="Where to verify">
  Go to **Plugins** and click your Telegram instance. The instance detail page shows webhook
  registration status, recent events, and delivery outcomes. This is the first place to check when
...
1. Open your Telegram plugin instance.
...
- Check **Activity** for work items. If work items are being created but responses fail, the issue is on the delivery side — look at the work item detail for errors.
