...
**Production:** Set your public URL in **Settings > Runtime > Public app URL** in the admin UI, or set `APP_BASE_URL` in your environment. Nitejar auto-registers the webhook with Telegram when you save the plugin instance. The runtime setting takes priority over environment variables. See the [configuration docs](/docs/start-here/configuration#public-app-url) for the full resolution order.
...
Or set it in **Settings > Runtime > Public app URL** in the admin UI.
