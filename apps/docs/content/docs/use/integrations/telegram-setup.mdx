...
<Callout type="warn" title="Important">
  ...
  in your `.env` file or clipboard is your responsibility.
</Callout>
...
Add the token to your `apps/web/.env` file:

```bash
TELEGRAM_BOT_TOKEN=123456789:ABCdefGhIjKlMnOpQrStUvWxYz
```

This sets a default token for new Telegram plugin instances. You can override it per-instance in the admin UI.

**Option B: Per-instance in admin**

Skip the env var and paste the token directly when creating a plugin instance (next step). This is the right approach if you run multiple Telegram bots.
