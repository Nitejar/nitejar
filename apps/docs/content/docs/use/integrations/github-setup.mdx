...
Or pass `appId` and `privateKey` directly in the plugin instance config. Both fields are stored encrypted at rest.
...
4. Fill in the configuration:
   - **Auth fields** — `appId` + `privateKey` for the GitHub App (unless you set them as env vars).
