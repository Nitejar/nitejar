...
description: Soul prompts, models, roles, memory, tools, and ownership.
---
...
See the [Tools Reference](/use/tools) for the full list with parameters and scope notes.
...
Multiple agents can share the same channel. They triage messages independently, claim exclusive turns, and silently pass when a message is not relevant. See [Agent Collaboration](/use/agent-collaboration) for the full guide on how agents work together.
...
<Callout type="info" title="Where to verify">
  Open **Agents** to inspect one agent's soul, model, memory, tools, queue mode, network policy,
  role assignments, and budget. Open **Company** when the question is about structure, staffing, or
  reusable role defaults instead of a single agent.
</Callout>
