...
| `APP_BASE_URL`          | No               | CLI: inferred from selected port      | Public URL where Nitejar is reachable. Used to construct webhook callback URLs. Can be overridden by the runtime setting in **Settings > Runtime**. See [Public app URL](#public-app-url). |
...
After starting Nitejar, open **Admin > Settings > Gateway** for model provider keys and **Admin > Settings > Capabilities > Tool Execution** for the Sprites key. Admin UI settings are the source of truth.
</Callout>
...
1. **Settings > Runtime > Public app URL** -- set in the admin UI, stored in the database. Takes highest precedence.
2. `APP_URL` env var
...
The runtime setting is the recommended approach for production. It persists across restarts and does not require redeploying or editing environment files. Set it at **Settings > Runtime** in the admin UI.
...
- **With ngrok:** Set `APP_BASE_URL=https://your-subdomain.ngrok-free.app` in your `.env`, or set the runtime URL in the admin UI.
...
Or set the URL in **Settings > Runtime > Public app URL** in the admin UI. The runtime setting takes priority over environment variables.
