...
Nitejar reads its configuration from `apps/web/.env`. Not root `.env`. Not `~/.nitejarrc`. One file.
...
pnpm --filter @nitejar/cli pack
npx --yes --package ./packages/nitejar-cli/nitejar-cli-0.1.0.tgz nitejar up --data-dir /tmp/nitejar-pack --port 3002
```

Most variables are optional. For local development with the default free model, you need exactly one thing: an `ENCRYPTION_KEY`. Everything else has a sane default or is only required when you enable the feature that needs it.
...
| `APP_BASE_URL`          | No               | `http://localhost:3000`               | Public URL where Nitejar is reachable. Used to construct webhook callback URLs. Must be a real URL in production.                                 |
...
| `TELEGRAM_BOT_TOKEN`    | No               | None                                  | Bot token from [@BotFather](https://t.me/botfather). Only needed if you're connecting a Telegram channel.                                         |
| `GITHUB_WEBHOOK_SECRET` | No               | None                                  | Secret for verifying GitHub webhook signatures. Required for GitHub integration.                                                                  |
| `GITHUB_APP_ID`         | No               | None                                  | GitHub App ID. Used with `GITHUB_PRIVATE_KEY` for production GitHub integration.                                                                  |
| `GITHUB_PRIVATE_KEY`    | No               | None                                  | GitHub App private key (PEM format). Pair with `GITHUB_APP_ID`.                                                                                   |
...
For the CLI path (`npx @nitejar/cli up`), migrations run automatically before the server starts.
