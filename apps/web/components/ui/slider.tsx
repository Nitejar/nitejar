...
const toNumberArray = (input: unknown) => {
  if (!Array.isArray(input)) {
    return null
  }

  return input.every((value) => typeof value === 'number') ? input : null
}
...
function Slider({
  ...
  const values = React.useMemo<number[]>(() => {
    const fromValue = toNumberArray(value)
    if (fromValue) {
      return fromValue
    }

    const fromDefaultValue = toNumberArray(defaultValue)
    if (fromDefaultValue) {
