...
type ChartPayload = Omit<RechartsPayload<ValueType, NameType>, 'payload'> & {
  payload?: Record<string, unknown> & { fill?: string }
}

type ChartLegendPayload = Omit<RechartsLegendPayload, 'payload' | 'dataKey' | 'value'> & {
  payload?: Record<string, unknown> & { value?: unknown }
...
function ChartTooltipContent({
  ...
  const safePayload = React.useMemo(() => (payload ?? []) as ChartPayload[], [payload])
...
function ChartLegendContent({
  ...
  const { config } = useChart()
  const safePayload = (payload ?? []) as ChartLegendPayload[]
